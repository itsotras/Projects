...
{
	VehicleMove* player = mOwner->GetComponent<VehicleMove>();
	VehicleMove* enemy = mOwner->GetGame()->GetEnemy()->GetComponent<VehicleMove>();
	if (player->currentLap > enemy->currentLap) {
		return true;
	}
	else if (player->currentLap < enemy->currentLap) {
		return false;
	}
	else {
		if (player->lastCheckpoint > enemy->lastCheckpoint) {
			return true;
		}
		else if (player->lastCheckpoint < enemy->lastCheckpoint) {
			return false;
		}
		else {
			if (player->DistanceToNextCheckpoint() < enemy->DistanceToNextCheckpoint()) {
				return true;
			}
			else {
				return false;
