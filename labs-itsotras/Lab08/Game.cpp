...
void Game::LoadData()
{
	...
	Mix_Chunk* raceStart = GetSound("Assets/Sounds/RaceStart.wav");
	...
	MeshComponent* trackComponent = new MeshComponent(mTrack);
	trackComponent->SetMesh(mRenderer->GetMesh("Assets/Track.gpmesh"));

	mEnemy = new Enemy(this);
	mPlayer->SetState(ActorState::Paused);
	mEnemy->SetState(ActorState::Paused);

	Mix_PlayChannel(-1, raceStart, 0);
}
