...
void LoadActor(const rapidjson::Value& actorValue, Game* game, Actor* parent)
{
	if (actorValue.IsObject())
	{
		...
		else if (type == "Player")
		{
			...
			Checkpoint* checkpt = new Checkpoint(game, parent);
			game->GetCheckpts().push(checkpt);
			checkpt->SetActive(false);
			std::string level;
			if (GetStringFromJSON(actorValue, "level", level))
			{
				checkpt->SetLevelString(level);
			}

			std::string text;
			if (GetStringFromJSON(actorValue, "text", text))
			{
				checkpt->SetTextString(text);
			}
			...
			{
				camera->SetInterpTime(interpTime);
			}

			float pauseTime;
			if (GetFloatFromJSON(actorValue, "pauseTime", pauseTime))
			{
				camera->SetPauseTime(pauseTime);
			}
		...
		if (actor)
		{
			...
			if (GetBoolFromJSON(actorValue, "mirror", isMirror)) {
				if (isMirror) {
					((Block*)actor)->SetMirror(isMirror);
				}
			}
			...
			if (GetBoolFromJSON(actorValue, "rotates", isRotate)) {
				if (isRotate) {
					((Block*)actor)->SetRotate(isRotate);
				}
			}
