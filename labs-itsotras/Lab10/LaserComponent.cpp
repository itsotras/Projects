...
LaserComponent::LaserComponent(Actor* owner) :MeshComponent(owner, true) {
	...
	//myLine.mStart = mOwner->GetPosition();
	//myLine.mEnd = mOwner->GetPosition() + 500.0f*mOwner->GetForward(mOwner->GetRotation());
}
...
void LaserComponent::Update(float deltaTime) {
	...
	LineSegment myLine;
	myLine.mStart = mOwner->GetPosition();
	myLine.mEnd = mOwner->GetPosition() + 500.0f * mOwner->GetQuatForward();
	CastInfo myInfo;
	Vector3 newDir = mOwner->GetQuatForward();
