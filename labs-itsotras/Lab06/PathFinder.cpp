...
{
	std::unordered_map<PathNode*, NodeInfo> myInfo;
	std::set<PathNode*> openSet;
	PathNode* currNode = start;
	myInfo[currNode].isClosedNode = true;
	do {
		for (auto adjNode : currNode->mAdjacent) {
			if (!myInfo[adjNode].isClosedNode) {
				if (openSet.find(adjNode) != openSet.end()) {
					float new_g = myInfo[currNode].g + Vector2::Distance(adjNode->GetPosition(), currNode->GetPosition());
					if (new_g < myInfo[adjNode].g) {
						myInfo[adjNode].mParent = currNode;
						myInfo[adjNode].g = new_g;
						myInfo[adjNode].f = myInfo[adjNode].h + myInfo[adjNode].g;
					}
				}
				else {
					myInfo[adjNode].mParent = currNode;
					myInfo[adjNode].h = Vector2::Distance(adjNode->GetPosition(), end->GetPosition());
					myInfo[adjNode].g = myInfo[currNode].g + Vector2::Distance(adjNode->GetPosition(), currNode->GetPosition());
					myInfo[adjNode].f = myInfo[adjNode].h + myInfo[adjNode].g;
		...
		float min_f = -1.0f;
		std::set<PathNode*>::iterator nextCurr;
		for (std::set<PathNode*>::iterator it = openSet.begin(); it != openSet.end(); ++it) {
			if (myInfo[*it].f < min_f) {
				nextCurr = it;
				min_f = myInfo[*it].f;
			}
			if (min_f == -1.0f) {
		...
		currNode = *nextCurr;
		openSet.erase(nextCurr);
		myInfo[currNode].isClosedNode = true;
	} while (currNode != end);

	PathNode* temp = end;
	while (temp != start) {
		outPath.push_back(temp);
		temp = myInfo[temp].mParent;
	}
