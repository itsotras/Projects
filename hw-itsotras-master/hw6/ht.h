...
struct LinearProber : public Prober<KeyType> {
    ...
    HASH_INDEX_T next() 
    {
        ...
        if( this->m_ == this->numProbes_) {//if numProbes is equal to the m_ value, return npos
            return this->npos; 
        }
...
void HashTable<K,V,Prober,Hash,KEqual>::insert(const ItemType& p)
{
    ...
    if((double)((double)total/ (double)table_.size()) >= this->alpha){//if we are above the alpha factor
        this->resize();
    }

    HASH_INDEX_T h = this->probe(p.first);//get the first nullptr or the first version of this item
    
    if(h == npos){
        throw std::logic_error("No available place");
    }
    else if(table_[h] == nullptr){//if there is nothing here
        table_[h] = new HashItem(p);//add a new item
        total++;
        actual++;
    }
    else{
...
{
    if(this->empty() == false){//only remove if we are not empty
        HashItem* myItem = this->internalFind(key);//get the first nullptr or the first version of this item
        if(myItem != nullptr){//if there is something here
            myItem->deleted = true;
            actual--;
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    this->mIndex_++;
    if(mIndex_ == 28){
        throw std::logic_error("Not enough space in capacity");
    }
    std::vector<HashItem*> oldtable = table_;
    table_.assign(CAPACITIES[mIndex_], nullptr);//resizes the table
    for(int i = 0; i<(int)oldtable.size(); i++){//loop through old_table
        
        if(oldtable[i] != nullptr){//if it is a member we need to probe or delete

            if(oldtable[i]->deleted == true){//if this is a deleted member
                total--;
                delete oldtable[i];//we can delete it

            }
            else{
                HASH_INDEX_T h= this->probe(oldtable[i]->item.first);
                table_[h] = oldtable[i];//set table at that new probe to this pointer
