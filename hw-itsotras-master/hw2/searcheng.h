...
private:
    ...
    //       their page/filename
    StringSet allMyTerms;
    StringSet allOutgoingLinks;
    std::map<std::string, WebPage*> file_WebPage; // to use for incoming and outgoing <filename, webpage>
    std::map<std::string, WebPageSet> myWebPages_; // a map of a strings as keys and the webpages that have those strings in a set as the values
